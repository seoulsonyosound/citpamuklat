  ...
  return (
    <header className="w-full bg-[#040912]/95 border-b border-amber-500/10 backdrop-blur-md sticky top-0 z-45 px-4 md:px-8 py-3.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {/* Navigation links */}
        <nav className="flex items-center gap-1 md:gap-3 bg-[#08111F]/70 border border-slate-850 p-1 rounded-2xl">
          {navLinks.map((link) => {
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute inset-0 bg-amber-600 rounded-xl shadow-lg shadow-amber-600/10 border border-amber-500/20 -z-10"
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="bg-[#0e1b30] border border-slate-800 hover:bg-slate-800 hover:border-slate-700 hover:text-red-400 p-2.5 rounded-xl text-slate-400 transition-all cursor-pointer flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider"
